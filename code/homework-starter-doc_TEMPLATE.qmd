...
**Specific Leaf Area (SLA) and root dry mass**

specific leaf area seem to be strongly positively correlated (Pearson's r0.858).

**Leaf Dry Mass and SLA**

Leaf dry mass and SLA are not correlated (Pearson's r = 0.033)

**Shoot Dry Mass and Total Dry Mass**

Shoot dry mass and total dry mass are strongly positively correlated (Pearson's r = 0.902)
...
**Plants that have higher biomass seem to have a larger range. Grindelia has the highest biomass while Acmispon has the lowest biomass.**
...
**Well watered treatments seem to have a larger range of total biomass than drought stressed ones, and appear to also have a larger total biomass. This makes sense based on how plants grow.**
...
**As specific leaf area increases, total biomass also seems to increase. This make sense because larger leaves will likely come from larger plants which would have more biomass.**

# Model construction

What is the null model?

0.  **Total mass is not predicted by any of the three predictor variables.**

What is the saturated model?

1.  **Total mass is predicted by SLA, water treatment, and species type.**
...
look at these diagnostics later!
...
From the `MuMIn` package (MuMIn stands for multimodel inference)
...
Out of the four models we have tried, the best model is the one that includes water treatment and species type.
...
-   reference for `water_treatmentDrought stressed`\
-   reference for `species_nameAcmispon glaber`
...
-   intercept: Mass of Acmispon glaber in drought stress treatment when SLA = 0\
    -   SLA slope: **across drought treatment and species, with each.1 unit increase in SLA (mm\^2/dry mass of leaf), we expect an ijncrease in total mass of -0.00025 WHICH MEANS WITH THE ESTIMATE AND SE THERE IS NO EFECT OF SLA ON TOTAL MASS**\
...
```{r}
model_predsexample <- ggpredict(model4, 
...
model_preds_for_plottingexample <- model_preds %>% 
